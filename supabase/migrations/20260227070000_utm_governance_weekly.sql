create table if not exists public.utm_governance_weekly (
  ...
  ok_rate numeric null,
  missing_rate numeric null,
  inserted_at timestamptz not null default now(),
  constraint utm_governance_weekly_unique unique (week_start, platform, campaign_id)
);

create index if not exists utm_governance_weekly_week_start_idx
  on public.utm_governance_weekly (week_start);

create or replace function public.compute_utm_governance_weekly(
  since_date date,
  until_date date
)
returns bigint
...
begin
  with filtered as (
    ...
    where mur.date_seen >= since_date
      ...
      and (
        mur.status = 'missing'
        or (nullif(btrim(mur.utm_campaign), '') is not null and nullif(btrim(mur.utm_campaign), '') ~* '^(per_|aw[_-])')
      )
    group by 1, 2, 3
  ),
  up as (
