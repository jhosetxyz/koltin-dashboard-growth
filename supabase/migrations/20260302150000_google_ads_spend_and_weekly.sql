-- Google Ads integration: unified spend table columns, mapping tables, weekly multi-platform spend.

-- Ensure ad_spend_daily supports Google + raw payloads.
alter table public.ad_spend_daily
  add column if not exists day date,
  add column if not exists raw_json jsonb,
  add column if not exists inserted_at timestamptz not null default now();

-- Backfill day for compatibility with older schemas / jobs.
update public.ad_spend_daily
set day = date
where day is null and date is not null;

-- Ensure campaign mapping exists and supports multiple platforms (meta + google).
create table if not exists public.campaign_mapping (
...
create index if not exists campaign_mapping_utm_campaign_idx
  on public.campaign_mapping (utm_campaign);

-- Optional snapshot table for Google campaigns (names/status/channel).
create table if not exists public.google_campaigns (
  ...
  inserted_at timestamptz not null default now(),
  constraint google_campaigns_uidx unique (customer_id, campaign_id)
);

create index if not exists google_campaigns_customer_id_idx
  on public.google_campaigns (customer_id);

-- Weekly: include multi-platform spend (meta + google) and attribute leads by utm_source.
