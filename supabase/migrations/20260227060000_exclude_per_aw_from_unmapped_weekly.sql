-- Exclude per_/aw_/aw- UTMs from legacy/sucio report.
-- IMPORTANT: This migration intentionally DOES NOT edit the original 20260227004000 migration.
-- It keeps the EXACT same function signature for public.compute_utm_unmapped_weekly(...).
...
as $$
  ...
  unmapped_leads as (
    ...
    where c.created_at >= since_date::timestamptz
      ...
      and nullif(btrim(c.utm_campaign), '') !~* '^(per_|aw[_-])'
      ...
      and not exists (
        select 1
        from active_equals e
        where e.pattern = nullif(btrim(c.utm_campaign), '')
      )
    group by 1, 2
  ),
  unmapped_spend as (
    ...
    where a.platform = 'meta'
      ...
      and nullif(btrim(cm.utm_campaign), '') !~* '^(per_|aw[_-])'
      ...
      and not exists (
        select 1
        from active_equals e
        where e.pattern = nullif(btrim(cm.utm_campaign), '')
      )
    group by 1, 2
  ),
  merged as (
...
-- Historical cleanup: remove per_/aw_/aw- rows that were previously inserted.
delete from public.utm_unmapped_weekly
where raw_utm ~* '^(per_|aw[_-])';
