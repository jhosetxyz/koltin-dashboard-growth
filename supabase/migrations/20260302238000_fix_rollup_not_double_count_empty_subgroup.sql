-- Fix rollup: avoid doubling when detail already has canonical_subgroup=''.
-- Strategy: roll up only non-empty subgroups into subgroup='' rows.
...
as $$
  ...
  merged_rollup as (
    select
      ...
      platform,
      canonical_campaign,
      ''::text as canonical_subgroup,
      sum(spend)::numeric as spend,
      sum(impressions)::bigint as impressions,
      sum(clicks)::bigint as clicks,
      sum(leads)::bigint as leads,
      sum(call_positive)::bigint as call_positive,
      sum(no_whats)::bigint as no_whats,
      sum(mql_count)::bigint as mql_count,
      sum(disqualified_count)::bigint as disqualified_count
    from merged_detail
    where canonical_subgroup <> ''
    group by 1,2,3
  ),
  merged_all as (
    select * from merged_detail
    union all
    select * from merged_rollup
  ),
  merged as (
    select
      week_start,
